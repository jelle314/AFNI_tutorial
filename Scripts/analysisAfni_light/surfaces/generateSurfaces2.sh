...
if [ ! -v THRNAME ] || [ -z "$THRNAME" ]
	then
	THRNAME="boundariesThr.nii.gz"
	SURFFOLDER="surfaces"
else
	SURFFOLDER="${THRNAME%%.*}"  #THRNAME without the extension
fi
...
do
    outputFileName=$(printf '%s[%s]' $THRNAME $i)
    if [ "$i" -lt 10 ] 
    then
...
genFolder=$(printf 'mkdir %s_folder' $SURFFOLDER)
moveTopo=$(printf 'mv *.1D.topo %s_folder/' $SURFFOLDER)
moveCoord=$(printf 'mv *.1D.coord %s_folder/' $SURFFOLDER)
moveSpec=$(printf 'mv spec.surfaces.smoothed %s_folder/' $SURFFOLDER)
$genFolder
$moveTopo
$moveCoord
$moveSpec	
